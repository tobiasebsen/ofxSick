...
CCommunicationHandler::CCommunicationHandler(CCommunication& rCommunication, CFramer& rFramer, 
  ...
  m_bLastRequestByIndex(false), m_LastSopasErrorCode(0), m_pFctCallback(0), m_pClassCallback(0)
{
 ...
 */
/*======================================================================================*/ 
void CCommunicationHandler::SetAsyncCallback(CCommunicationHandler::SopasAsyncCallback pCallback)
{
 ...
 */
/*======================================================================================*/
void CCommunicationHandler::HandleAsyncAnswer(std::auto_ptr<CBinaryDataStream>& rAnswer, SopasAsyncCallback pTemporaryCallback)
{
    ...
    if (answer.eAnswerType != SDecodedAnswer::INVALID_ANSWER)
    {
      ...
      if(answer.eAnswerType == SDecodedAnswer::METHOD_RESULT)
      {
        ...
        if(iter != m_AsyncMethodRequests.end())
        {
          ...
          {
            pSelectedCallback(*iter, *rAnswer, m_Protocol.GetDeserializer());
          }
          else
          {
 ...
 */
/*======================================================================================*/ 
void CCommunicationHandler::PollAsyncAnswers(int iPollTimeMs, SopasAsyncCallback pCallback)
{
  // Communication timeout to wait for an answer
  clock_t time_now = clock();
    ...
    if(coReceivedFramedData.get() != 0)
    {
      ...
      do
      {
        ...
        if (coUnframedData.get() != 0)
        {
          ...
          HandleAsyncAnswer(coUnframedData, pCallback);
        }
      } while (coReceivedFramedData->IsReadPosAtEnd() == false);
    }
