...
/*======================================================================================*/
bool CLms100SopasInterface::WriteAll()
{
    bool bRetVal = false;
    
    CBinaryDataStream* pReceivedData = m_Protocol.InvokeMethod("mEEwriteall", 0);
    if (pReceivedData != 0)
    {
        // Get return value of method
        CDeserializer& rDeserializer = m_Protocol.GetDeserializer();
        SOPAS_UInt8 eWriteAllError;
        if (rDeserializer.Deserialize(*pReceivedData, eWriteAllError) == true)
        {
        }
